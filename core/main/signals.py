...
def notify_about_contac(sender, instance, created, **kwargs):
	staffs = Staff.objects.all()
	# make a list of all phone numbers of staff numbers
...
def notify_about_appointment(sender, instance, created, **kwargs):
	...
	customer_message = f"Dear {instance.customer_name}, your appointment for{instance.service}({instance.service_type}) on {instance.date} at {instance.time} has been received. We will get back to you shortyl"
...
def send_sms(sender:str, message:str, receipients: array.array):
	header = {"api-key": settings.ARKESEL_API_KEY, 'Content-Type': 'application/json',
              'Accept': 'application/json'}
	
	SEND_SMS_URL = "https;//sms.arkesel.com/api/v2/sms/sned"
	payload = {
		"sender": sender,
		"message": message,
